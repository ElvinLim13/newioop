                ...
                using (SqlCommand commandCheckId = new SqlCommand(queryCheckId, connection))
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(trainingLevel))
                                updateCommand.Parameters.AddWithValue("@level", trainingLevel);
                            if (!string.IsNullOrWhiteSpace(trainingFe))
                            {
                                // 检查Training Fee是否是数字
                                decimal fee;
                                ...
                                {
                                    // 格式化Training Fee为带RM单位的字符串，最多两位小数
                                    string formattedFee = "RM " + fee.ToString("#.##");
                                    updateCommand.Parameters.AddWithValue("@Fee", formattedFee);
                                }
                            }
                            if (!string.IsNullOrWhiteSpace(timeStart))
                            {
                        ...
                        MessageBox.Show("Updated information!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return; // 停止保存操作
                    }
                    else
                    {
